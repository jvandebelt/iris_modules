                ...
                "0.1")                      // version
{
		...
		registerParameter("gain", "The transmit gain", "0", true, gain_x), Interval<int>(-1,42);
		...
		//registerParameter("antenna", "Antenna selection","",false,antenna_x);
		//registerParameter("bw", "Bandwidth in Hz","0",false,bw_x);
			...
			if(device_index_x < rtlsdr_get_device_count())
			{
				...
				*/
				...
				if (count > 0)
				{
					...
					{		
						range += rtl::range_t(gains[i] / 10.0);
					}
				...
				{																		
					...
					rtlsdr_set_tuner_gain(rtl_dev, int(range.clip(gain_x) * 10.0) );
				...
				if (bw_x > 0)
				{
				    ...
				    LOG(LINFO) << "Actual RX Bandwidth: " << bw_x/1e6 << " MHz...";
				}
				*/
 ...
 void RtlRxComponent::process()
    {
		...
		currentTimestamp_d = currentTimestamp_d + time_spec_t(0, outputBlockSize_x, rate);
		
		writeDataSet->sampleRate = rate;
		writeDataSet->timeStamp = currentTimestamp_d.get_real_secs();
    ...
    {
      std::vector< rtl::gain_range_t > if_gains;
      
      if_gains += rtl::gain_range_t(-3, 6, 9);
      if_gains += rtl::gain_range_t(0, 9, 3);
      if_gains += rtl::gain_range_t(0, 9, 3);
      if_gains += rtl::gain_range_t(0, 2, 1);
      if_gains += rtl::gain_range_t(3, 15, 3);
      if_gains += rtl::gain_range_t(3, 15, 3);
